...
function generateNumber(prefix: string, index: number) {
...
async function main() {
    ...
    for (let i = 0; i < 45; i++) {
        ...
        if (r < 0.1) {
            ...
            status = InvoiceStatus.PAID; // 40% are paid
        }
        ...
        const invoice = await prisma.invoice.create({
            data: {
                ...
                total,
                currency: 'MAD',
                notes: status === InvoiceStatus.OVERDUE ? 'Second reminder sent.' : 'Standard service delivery.',
                items: {
                    create: items,
                },
            },
