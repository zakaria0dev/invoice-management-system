...
export const createRole = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const createdRole = await prisma.userRole.findUnique({
            where: { id: role.id },
            include: {
                permissions: {
...
export const updateRole = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const roleWithPerms = await prisma.userRole.findUnique({
...
export const deleteRole = async (req: any, res: Response, next: NextFunction) => {
    try {
        const { id } = req.params;
        ...
        if (currentUserRoleId === id) {
            return next(new AppError('You cannot delete your own role', 400));
        }

        if (role._count.users > 0) {
            return next(new AppError('Cannot delete role with assigned users. Reassign users first.', 400));
        }
