...
const getStats = async (req, res, next) => {
    try {
        const results = await Promise.all([
            ...
            prisma_1.default.invoice.count({ where: { NOT: { status: 'CANCELLED' } } }),
            ...
            prisma_1.default.invoiceItem.groupBy({
                by: ['productId'],
                _count: { _all: true },
        ...
        const [totalRevenue, totalInvoices, overdueInvoices, totalClients, statusCounts, monthlyRevenue, rawTopClients, rawTopProducts] = results;
        ...
        // Get Names
        const topClientsWithNames = await Promise.all(rawTopClients.map(async (c) => {
            const client = await prisma_1.default.client.findUnique({ where: { id: c.clientId }, select: { name: true } });
            return { ...c, name: client?.name || 'Unknown' };
        }));
        const topProductsWithNames = await Promise.all(rawTopProducts.map(async (p) => {
            if (!p.productId)
                return { ...p, name: 'Custom Item' };
            const product = await prisma_1.default.product.findUnique({ where: { id: p.productId }, select: { name: true } });
            return { ...p, name: product?.name || 'Unknown' };
        }));
        const totalRefundsValue = (await prisma_1.default.refund.aggregate({
        ...
        const allPendingInvoices = await prisma_1.default.invoice.findMany({
            where: {
                AND: [
                    { NOT: { status: 'PAID' } },
                    { NOT: { status: 'CANCELLED' } },
            ...
            data: {
                ...
                cashFlow: (() => {
                    ...
                    monthlyRevenue.forEach((p) => {
                        ...
                        if (p.isRefund)
                            entry.expense -= Number(p.amount);
                        else
                            entry.income += Number(p.amount);
                    });
                    return Array.from(cashFlowMap.values()).sort((a, b) => a.sortDate - b.sortDate);
                })(),
