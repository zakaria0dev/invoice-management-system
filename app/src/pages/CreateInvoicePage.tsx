...
export default function CreateInvoicePage() {
    ...
    const handlePOSAddProduct = (product: any) => {
        ...
        if (currentItems.length === 1 && !currentItems[0].productId) {
            setValue(`items.0.productId`, product.id.toString(), { shouldValidate: true, shouldDirty: true });
    ...
    const onSubmit = async (data: InvoiceFormData) => {
        try {
            ...
            await addInvoice({ ...data, items: formattedItems, clientId: parseInt(data.clientId) });
            toast({ title: t('common.success'), description: t('invoices.create_success', 'Invoice created successfully!') });
            navigate('/invoices');
        } catch (error: any) {
            toast({ title: t('common.error'), description: error.message || t('invoices.create_error', 'Failed to create invoice.'), variant: 'destructive' });
        }
    };
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                        ...
                        <div className="flex items-center gap-2 p-3 bg-primary/5 border border-primary/10 rounded-lg text-xs text-primary/80 font-semibold shadow-inner">
                            <input type="checkbox" {...register('remindersEnabled')} id="reminders" className="h-4 w-4 rounded" />
                            <label htmlFor="reminders" className="cursor-pointer select-none">{t('invoices.enable_reminders')}</label>
                        </div>

                        <div className="flex gap-3 pt-4 border-t sticky bottom-0 bg-background pb-4 pt-6 z-10">
                            <Button type="button" variant="outline" className="flex-1 h-12 text-xs font-bold uppercase tracking-wider" onClick={() => navigate('/invoices')}>{t('invoices.cancel')}</Button>
                            <Button type="submit" className="flex-1 h-12 text-xs font-bold uppercase tracking-wider shadow-md hover:shadow-lg transition-all" disabled={isSubmitting}>
                                {isSubmitting ? t('common.saving') : t('invoices.create')}
                            </Button>
                        </div>
                    </form>
                </div>
